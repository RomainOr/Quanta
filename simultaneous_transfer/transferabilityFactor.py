...
#Datasets
(trainingSetSource, trainingLabelsSource),(testSetSource, testLabelsSource) = tf.keras.datasets.cifar10.load_data()
(trainingSetTarget, trainingLabelsTarget),(testSetTarget, testLabelsTarget) = tf.keras.datasets.cifar100.load_data()
...
def convBlock(pInput, size, transferLayers=[]):
	...
	if (transferLayers != []): x = binaryChoiceActivation('elu', initScalarValue, etaTF/eta)([transferLayers[0], x])
	else: x = tf.keras.layers.Activation('elu')(x)
	x = tf.keras.layers.BatchNormalization()(x)

	x = tf.keras.layers.Conv2D(size, 3, padding='same')(x)
	if (transferLayers != []): x = binaryChoiceActivation('elu', initScalarValue, etaTF/eta)([transferLayers[1], x])
	else: x = tf.keras.layers.Activation('elu')(x)
	x = tf.keras.layers.BatchNormalization()(x)
...
def createModel(placeholder, outputSize, withTransfer=None):
	lay = []
	if (withTransfer != None): lay = [withTransfer.layers[i].output for i in range(len(withTransfer.layers)) if type(withTransfer.layers[i]).__name__ in ['Conv2D', 'Dense']]	
	x = placeholder
	if lay != []: x = convBlock(x, 64, [lay[0], lay[1]])
	else : x = convBlock(x, 64)
	
	if lay != []: x = convBlock(x, 128, [lay[2], lay[3]])
	else : x = convBlock(x, 128)
	
	if lay != []: x = convBlock(x, 512, [lay[4], lay[5]])
	else : x = convBlock(x, 512)
...
NNSource = tf.keras.Model(inputs=inputPlaceholderSource, outputs=createModel(inputPlaceholderSource, 10)) 
NNSourceCopy = tf.keras.Model(inputs=inputPlaceholderTarget, outputs=createModel(inputPlaceholderTarget, 10)) 
NNTarget = tf.keras.Model(inputs=inputPlaceholderTarget, outputs=createModel(inputPlaceholderTarget, 100, withTransfer=NNSourceCopy)) 
NNWitness = tf.keras.Model(inputs=inputPlaceholderTarget, outputs=createModel(inputPlaceholderTarget, 100)) 
...
nbrLambdas = len([NNTarget.layers[i].get_weights() for i in range(len(NNTarget.layers)) if (type(NNTarget.layers[i]).__name__ == "binaryChoiceActivation")])
...
def lambdasMonitoring(b, w): 
	if (b %10 == 0 and b is not 0): 
		...
		for i in range(0, nbrLambdas): lambdas[i]+= [m.exp(weights[i][0])/(m.exp(weights[i][0])+m.exp(weights[i][1]))]

scalarWeightCallback = tf.keras.callbacks.LambdaCallback(on_batch_end=lambda batch,logs: lambdasMonitoring(batch, [NNTarget.layers[i].get_weights() for i in range(len(NNTarget.layers)) if (type(NNTarget.layers[i]).__name__ == "binaryChoiceActivation")]))

printLambdas = tf.keras.callbacks.LambdaCallback(on_epoch_end=lambda batch,logs: print("  TF : "+str(lambdas[0][-1])+"..."+str(lambdas[-1][-1])))


		
#Training and evaluating models
def train(modelName, dataAugmentation=False, fromPreviousTraining=False): #modelName : T for target, S for source, W for witness
	if (modelName == 'T'):
	...
	if dataAugmentation: 
		...
		model.fit_generator(trainGenerator.flow(trainingSet, trainingLabels, batch_size=bc), epochs=noe, callbacks=cb, )
	else: 
		if fromPreviousTraining: model.load_weights(weights)
